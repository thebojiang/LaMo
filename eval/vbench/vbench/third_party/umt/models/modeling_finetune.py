...
def vit_large_patch16_224(pretrained=False, **kwargs):
    ...
    # timm>=1.0 auto-injects ``cache_dir`` into create_model kwargs. The
    # VisionTransformer below has fixed positional signature (no **kwargs sink),
    # so without this pop we get:
    #   TypeError: VisionTransformer.__init__() got an unexpected keyword argument 'cache_dir'
    # which kills the whole ``human_action`` torchrun group on every node.
