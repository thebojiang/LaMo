...
class PTDCheckpointManager:
    def __init__(
        ...
        after_save_callback: Optional[Callable[[str, int], None]] = None,
    ) -> None:
        self.states = states
        self.states.update(
            {
    ...
    def save(self, step: int = -1, force: bool = False, *, _device: torch.device, _is_main_process: bool) -> str:
        ...
        if self._callback_fn is not None:
            for state_dict in state_dicts:
                self._callback_fn(state_dict, step)

        if self.after_save_callback is not None:
            if _is_main_process:
                try:
                    self.after_save_callback(checkpoint_dir.as_posix(), step)
                except Exception as e:
                    logger.exception("after_save_callback failed: %s", e)
