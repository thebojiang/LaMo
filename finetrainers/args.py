...
class BaseArgs:
    ...
    For helpful information about arguments, run `python train/train.py --help`.

    TODO(aryan): add `python train/train.py --recommend_configs --model_name <model_name>` to recommend
    good training configs for a model after extensive testing.
    TODO(aryan): add `python train/train.py --memory_requirements --model_name <model_name>` to show
    ...
    ---------------
    model_name (`str`):
        Name of model to train. To get a list of models, run `python train/train.py --list_models`.
    ...
    dataset_config (`str`):
        ...
        dictionary must contain the following keys:
            ...
            - "short_video_handling": (`str`, defaults to `"skip"`)
                When a video has fewer frames than the smallest bucket: "skip" to drop the sample, "pad" to
                temporally pad (cyclic repeat) to the nearest bucket. Use "skip" to avoid training on padded content.
    ...
    # LaMo model arguments
    predictor_in_channels: int = 16
    predictor_hidden_channels: int = 256
    predictor_num_res_blocks: int = 8
    predictor_use_se: int = 1
    predictor_use_prev_delta: int = 0
    predictor_use_prompt_cond: int = 1
    ...
    enable_denoiser_motion_loss: int = 0
    ...
    adapter_weight_dir: Optional[str] = None
    ...
    run_validation: bool = True  # Set False (e.g. via --no_validation) to skip validation stage
    ...
    output_dir: str = None
    output_dir_sync_path: Optional[str] = None  # Legacy compatibility only; local filesystem sync is not supported.
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        training_arguments = {
            ...
            "adapter_weight_dir": self.adapter_weight_dir,
        ...
        miscellaneous_arguments = {
            ...
            "output_dir_sync_path": self.output_dir_sync_path,
...
def _add_model_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    # LaMo model arguments
    parser.add_argument("--predictor_in_channels", type=int, default=16, help="LaMo: predictor input channels")
    parser.add_argument("--predictor_hidden_channels", type=int, default=256, help="LaMo: predictor hidden channel width")
    parser.add_argument("--predictor_num_res_blocks", type=int, default=8, help="LaMo: number of predictor residual blocks")
    parser.add_argument("--predictor_use_se", type=int, default=1, help="LaMo: enable Squeeze-and-Excitation blocks")
    parser.add_argument("--predictor_use_prev_delta", type=int, default=0, help="LaMo: concatenate previous latent delta as predictor input")
    parser.add_argument("--predictor_use_prompt_cond", type=int, default=1, help="LaMo: enable prompt FiLM conditioning in the predictor")
    parser.add_argument("--prompt_text_dim", type=int, default=4096, help="LaMo: pooled prompt embedding dimension")
    parser.add_argument("--prompt_cond_dim", type=int, default=128, help="LaMo: prompt FiLM bottleneck dimension")
    parser.add_argument("--prompt_dropout_prob", type=float, default=0.1, help="LaMo: prompt dropout probability during predictor training")
    parser.add_argument("--predictor_lr", type=float, default=1e-3, help="LaMo: predictor learning rate")
    parser.add_argument("--predictor_lr_scheduler", type=str, default="cosine", help="LaMo: predictor LR scheduler")
    parser.add_argument("--predictor_noise_aug_prob", type=float, default=0.5, help="LaMo: latent noise augmentation probability")
    parser.add_argument("--predictor_noise_aug_scale", type=float, default=1.0, help="LaMo: latent noise augmentation scale")
    parser.add_argument("--predictor_cosine_weight", type=float, default=0.5, help="LaMo: cosine direction loss weight")
    parser.add_argument("--enable_denoiser_motion_loss", type=int, default=0, help="LaMo: enable denoiser-side BMV motion loss")
    parser.add_argument("--lambda_bmv_rel_l2", type=float, default=0.0, help="LaMo: BMV relative L2 motion loss weight")
    parser.add_argument("--bmv_tau", type=int, default=2, help="LaMo: temporal lag for BMV motion loss")
    parser.add_argument("--guidance_lambda", type=float, default=15.0, help="LaMo: inference guidance strength")
    parser.add_argument("--guidance_step_ratio", type=float, default=0.8, help="LaMo: fraction of denoising steps with guidance")
...
def _add_dataset_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--hash_save", action="store_true")
    parser.add_argument("--first_samples", action="store_true")
...
def _add_training_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to exported weight dir (e.g. output_dir/weight_1000) to load LoRA and, for LaMo, physical_module.safetensors at training start.",
    )
...
def _add_validation_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--no_validation", action="store_true", help="Disable validation stage (no validation during or after training)")
...
def _add_miscellaneous_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--output_dir_sync_path", type=str, default=None, help="Legacy compatibility option; local filesystem checkpoint sync is not supported.")
...
def _map_to_args_type(args: Dict[str, Any]) -> BaseArgs:
    """Copy argparse Namespace fields onto BaseArgs.

    train/train.py reads the returned BaseArgs (not the raw Namespace). Any new
    ``parser.add_argument`` in ``_add_model_arguments`` must assign the same
    dest here; otherwise CLI overrides are ignored (defaults only).
    """
    ...
    # LaMo model arguments
    result_args.predictor_in_channels = getattr(args, "predictor_in_channels", 16)
    result_args.predictor_num_res_blocks = getattr(args, "predictor_num_res_blocks", 8)
    result_args.predictor_hidden_channels = getattr(args, "predictor_hidden_channels", 256)
    result_args.predictor_use_se = getattr(args, "predictor_use_se", 1)
    result_args.predictor_use_prev_delta = getattr(args, "predictor_use_prev_delta", 0)
    result_args.predictor_use_prompt_cond = getattr(args, "predictor_use_prompt_cond", 1)
    result_args.prompt_text_dim = getattr(args, "prompt_text_dim", 4096)
    result_args.prompt_cond_dim = getattr(args, "prompt_cond_dim", 128)
    result_args.prompt_dropout_prob = getattr(args, "prompt_dropout_prob", 0.1)
    result_args.predictor_lr = getattr(args, "predictor_lr", 1e-3)
    result_args.predictor_lr_scheduler = getattr(args, "predictor_lr_scheduler", "cosine")
    result_args.predictor_noise_aug_prob = getattr(args, "predictor_noise_aug_prob", 0.5)
    result_args.predictor_noise_aug_scale = getattr(args, "predictor_noise_aug_scale", 1.0)
    result_args.predictor_cosine_weight = getattr(args, "predictor_cosine_weight", 0.5)
    result_args.enable_denoiser_motion_loss = getattr(args, "enable_denoiser_motion_loss", 0)
    result_args.lambda_bmv_rel_l2 = getattr(args, "lambda_bmv_rel_l2", 0.0)
    result_args.bmv_tau = getattr(args, "bmv_tau", 2)
    result_args.guidance_lambda = getattr(args, "guidance_lambda", 15.0)
    result_args.guidance_step_ratio = getattr(args, "guidance_step_ratio", 0.8)
    ...
    result_args.hash_save = args.hash_save
    result_args.first_samples = args.first_samples
    ...
    result_args.adapter_weight_dir = getattr(args, "adapter_weight_dir", None)
    ...
    result_args.run_validation = not getattr(args, "no_validation", False)
    ...
    result_args.output_dir_sync_path = getattr(args, "output_dir_sync_path", None)
