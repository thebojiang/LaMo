...
class PytorchDTensorParallelBackend(BaseParallelBackend):
    def __init__(
        ...
        self._we_initialized_dist = False
        ...
        # Avoid "trying to initialize the default process group twice" when the cluster
        # (e.g. Slurm srun / platform launcher) has already initialized the process group.
        if not torch.distributed.is_initialized():
            torch.distributed.init_process_group(
                backend=self._backend, timeout=datetime.timedelta(seconds=self._timeout)
            )
            self._we_initialized_dist = True
    ...
    def prepare_dataloader(
        ...
        dataloader = DPDataLoader(
            dp_local_rank, dataset, batch_size=batch_size, num_workers=num_workers, pin_memory=pin_memory
        )
    ...
    def destroy(self):
        if self.is_main_process:
            self.tracker.finish()
        if self._we_initialized_dist and torch.distributed.is_initialized():
