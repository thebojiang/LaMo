...
def __initialize(fork=False, backend='nccl', gpu_id_if_not_distibuted=0, timeout_minutes=30, train_on_cluster=False):
    global __device
    ...
    """ # Do not know why it is commented, but nuplan cannot work without it
...
def init_distributed_mode(local_out_path, fork=False, only_sync_master=False, timeout_minutes=30, train_on_cluster=False):
    try:
        __initialize(fork=fork, timeout_minutes=timeout_minutes, train_on_cluster=train_on_cluster)
        barrier()
    ...
    _change_builtin_print(is_master())
    if not train_on_cluster and (is_master() if only_sync_master else is_local_master()) and local_out_path is not None and len(local_out_path):
    ...
    def prt(*args, **kwargs):
        ...
        if is_master or force:
            if not clean:
                ...
                time_str = datetime.datetime.now(tz=pytz.timezone('America/Los_Angeles')).strftime('[%m-%d %H:%M:%S]')
                builtin_print(f'{time_str} ({file_desc}, line{f_back.f_lineno:-4d})=>', *args, **kwargs)
            else:
                builtin_print(*args, **kwargs)
        else:
            __builtin__.print = lambda *args, **kwargs: None
